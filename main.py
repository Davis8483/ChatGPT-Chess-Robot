...
def _create_aliases():
  ...
  ptg.tim.define("!pos_x", lambda *_:str(chess_bot.target_x))
  ptg.tim.define("!pos_y", lambda *_:str(chess_bot.target_y))
  ptg.tim.define("!pos_z", lambda *_:str(chess_bot.target_z))
...
def save_prompt(page, save, _dosave=None):
  ...
  if _dosave == None:
    ...
    try: save_menu.close(animate=False)
    except: pass
    navigate_menu(page)
...
def navigate_menu(page):
  ...
  if page == "gpt_settings":
    api_key_input = ptg.InputField(value=settings["gpt"]["api-key"], prompt="API key: ")
    prompt_input = ptg.InputField(value=settings["gpt"]["prompt"], prompt="Prompt: ")
    menu = ptg.Window(
      ...
      "",
      ["Back", lambda *_: save_prompt("settings", save={"gpt": {"api-key": api_key_input.value, "prompt": prompt_input.value}})],
      is_static=True,
      is_noresize=True,
      vertical_align=0,
  ...
  if page == "hardware_settings":
    ...
    menu = ptg.Window(
      ...
      "",
      ["Back", lambda *_: save_prompt("settings", save={"hardware":{"serial-port": serial_port_input.value, "baud-rate": baud_rate_input.value}})],
      is_static=True,
      is_noresize=True,
      vertical_align=0,
    ...
    menu = ptg.Window(
      ...
      ptg.Container(
        ptg.Splitter(
          ...
          ptg.Label("([!pos_x] [/!], [!pos_y] [/!]) mm", parent_align=2)
        ),
        ptg.KeyboardButton("w ↑", lambda *_:chess_bot.goto_position(chess_bot.target_x, (chess_bot.target_y + xy_steps()), chess_bot.target_z), bound="w"),
        ptg.KeyboardButton("a ←", lambda *_:chess_bot.goto_position((chess_bot.target_x - xy_steps()), chess_bot.target_y, chess_bot.target_z), bound="a"),
        ptg.KeyboardButton("s ↓", lambda *_:chess_bot.goto_position(chess_bot.target_x, (chess_bot.target_y - xy_steps()), chess_bot.target_z), bound="s"),
        ptg.KeyboardButton("d →", lambda *_:chess_bot.goto_position((chess_bot.target_x + xy_steps()), chess_bot.target_y, chess_bot.target_z), bound="d"),
        "",
        ptg.Splitter(
      ...
      ptg.Container(
        ...
        ptg.KeyboardButton("e Up", lambda *_:chess_bot.goto_position(chess_bot.target_x, chess_bot.target_y, (chess_bot.target_z + z_steps())), bound="e"),
        ptg.KeyboardButton("q Down", lambda *_:chess_bot.goto_position(chess_bot.target_x, chess_bot.target_y, (chess_bot.target_z - z_steps())), bound="q"),
        ptg.Button("Home", lambda *_:chess_bot.goto_position(chess_bot.target_x, chess_bot.target_y, 0)),
        "",
        ptg.Splitter(
          ptg.Label("[app.label]Step:", parent_align=0),
      ...
      ptg.Container(
        ...
        ptg.KeyboardButton("r Open", lambda *_:chess_bot.set_grabber('open'), bound="r"),
        ptg.KeyboardButton("f Close", lambda *_:chess_bot.set_grabber('closed'), bound="f"),
        "",
        relative_width=0.6
      ),
      "",
